...
async function main() {
  ...
  //   cuisines.find((cuisine: any) => cuisine.name === "indian")?.id || 1;
  // const mexicanCuisineId =
  //   cuisines.find((cuisine: any) => cuisine.name === "mexican")?.id || 1;
  // const italianCuisineId =
  //   cuisines.find((cuisine: any) => cuisine.name === "italian")?.id || 1;

  // const ottawaLocationId =
  //   locations.find((location: any) => location.name === "ottawa")?.id || 1;
  // const torontoLocationId =
  //   locations.find((location: any) => location.name === "toronto")?.id || 1;
  // const niagaraLocationId =
  //   locations.find((location: any) => location.name === "niagara")?.id || 1;
  ...
  const vivaanId =
    restaurants.find(
      (restaurant: any) => restaurant.name === "Vivaan - fine Indian"
    )?.id || 1;
  const RamaKrishnaId =
    restaurants.find(
      (restaurant: any) => restaurant.name === "RamaKrishna Indian"
    )?.id || 1;
  const coconutLagoonId =
    restaurants.find((restaurant: any) => restaurant.name === "Coconut Lagoon")
      ?.id || 1;
  const lastTrainToDelhiId =
    restaurants.find(
      (restaurant: any) => restaurant.name === "Last Train to Delhi"
    )?.id || 1;
  const adrakYorkvilleId =
    restaurants.find((restaurant: any) => restaurant.name === "Adrak Yorkville")
      ?.id || 1;
  const curryishTavernId =
    restaurants.find((restaurant: any) => restaurant.name === "Curryish Tavern")
      ?.id || 1;
  const utsavId =
    restaurants.find((restaurant: any) => restaurant.name === "Utsav")?.id || 1;
  const pukkaId =
    restaurants.find((restaurant: any) => restaurant.name === "Pukka")?.id || 1;
  const kamasutraIndianId =
    restaurants.find(
      (restaurant: any) => restaurant.name === "Kamasutra Indian"
    )?.id || 1;
  const eldoradoTacoId =
    restaurants.find((restaurant: any) => restaurant.name === "Eldorado Taco")
      ?.id || 1;
  const laBartolaId =
    restaurants.find((restaurant: any) => restaurant.name === "La Bartola")
      ?.id || 1;
  const elCatrinId =
    restaurants.find((restaurant: any) => restaurant.name === "El Catrin")
      ?.id || 1;
  const mariachisId =
    restaurants.find((restaurant: any) => restaurant.name === "3 Mariachis")
      ?.id || 1;
  const canoRestaurantId =
    restaurants.find((restaurant: any) => restaurant.name === "Cano Restaurant")
      ?.id || 1;
  const bluRistoranteId =
    restaurants.find((restaurant: any) => restaurant.name === "Blu Ristorante")
      ?.id || 1;
  const stelvioId =
    restaurants.find((restaurant: any) => restaurant.name === "Stelvio")?.id ||
    1;
  const sofiaId =
    restaurants.find((restaurant: any) => restaurant.name === "Sofia")?.id || 1;
  ...
  const userRamiz = await prisma.user.create({
    data: {
      first_name: "Ramiz",
      last_name: "Ahmed",
  ...
  const userKhayyam = await prisma.user.create({
    data: {
      first_name: "Khayyam",
      last_name: "Ali",
      email: "khayyam@gmail.com",
      city: "toronto",
      password: "$2b$10$I8xkU2nQ8EAHuVOdbMy9YO/.rSU3584Y.H4LrpIujGNDtmny9FnLu",
      phone: "1112223333",
    },
  ...
  const userTony = await prisma.user.create({
    data: {
      first_name: "Tony",
      last_name: "Soprano",
  ...
  const userWalter = await prisma.user.create({
    data: {
      first_name: "Walter",
      last_name: "White",
